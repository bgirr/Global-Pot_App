...
static uString* STRINGS[143];
static uType* TYPES[1];

namespace g{

// C:\Users\EliteBook-User\AppData\Local\Fusetools\Packages\Fuse.Controls\0.44.1\.uno\package
// ------------------------------------------------------------------------------------------
...
// C:\Users\EliteBook-User\AppData\Local\Fusetools\Packages\Fuse.Controls.Panels\0.44.1\.uno\package
// -------------------------------------------------------------------------------------------------
...
// C:\Users\EliteBook-User\AppData\Local\Fusetools\Packages\Fuse.Controls.Primitives\0.44.1\.uno\package
// -----------------------------------------------------------------------------------------------------
...
{
    ::STRINGS[28] = uString::Const("uniform mat4 f,b;uniform vec2 c,d;attribute vec2 a;varying vec2 g;void main(){g=(a*c)+d;gl_Position=b*(f*vec4(vec3(a,float(0)),1.));}");
    ::STRINGS[29] = uString::Const("uniform vec4 e;uniform sampler2D h;varying vec2 g;void main(){gl_FragColor=texture2D(h,g)*e;}");
    ::STRINGS[4] = uString::Const("a");
    ::STRINGS[2] = uString::Const("b");
    ::STRINGS[3] = uString::Const("c");
    ::STRINGS[5] = uString::Const("d");
...
// C:\Users\EliteBook-User\AppData\Local\Fusetools\Packages\Fuse.Controls.Primitives\0.44.1\.uno\ux11\$.uno
// --------------------------------------------------------------------------------------------------------
...
// C:\Users\EliteBook-User\AppData\Local\Fusetools\Packages\Fuse.Controls.Video\0.44.1\.uno\package
// ------------------------------------------------------------------------------------------------
...
{
    ::STRINGS[30] = uString::Const("uniform float c,d,e,f,g,h,k,l,m,n;uniform mat4 i,j;uniform vec2 o;attribute vec3 a,b;varying vec2 p;void main(){p=vec2(((a.x*c)+(a.y*k))+(a.z*l),((b.x*f)+(b.y*m))+(b.z*n))/o;gl_Position=j*(i*vec4(vec2(((a.x*c)+(a.y*d))+(a.z*e),((b.x*f)+(b.y*g))+(b.z*h)),float(0),float(1)));}");
    ::STRINGS[31] = uString::Const("uniform sampler2D q;varying vec2 p;void main(){gl_FragColor=vec4(texture2D(q,p).xyz,1.);}");
    ::STRINGS[4] = uString::Const("a");
    ::STRINGS[2] = uString::Const("b");
    ::STRINGS[3] = uString::Const("c");
    ::STRINGS[5] = uString::Const("d");
...
// C:\Users\EliteBook-User\AppData\Local\Fusetools\Packages\FuseCore\0.44.1\.uno\package
// -------------------------------------------------------------------------------------
...
// C:\Users\EliteBook-User\AppData\Local\Fusetools\Packages\Fuse.Drawing\0.44.1\.uno\package
// -----------------------------------------------------------------------------------------
...
// C:\Users\EliteBook-User\AppData\Local\Fusetools\Packages\Fuse.Drawing.Primitives\0.44.1\.uno\package
// ----------------------------------------------------------------------------------------------------
...
{
    ::STRINGS[38] = uString::Const("uniform vec2 b,c,e,f,g,n;uniform mat4 d;uniform float o;attribute vec2 a;varying float s;varying vec2 t;float v(vec2 w,vec2 x,float y){vec2 z=(w*e)-f;float A=dot(z,x)/y;return (A-g.x)/g.y;}void main(){vec2 w=a*b;vec2 x=w+c;s=v(x/e,n,o);t=w;gl_Position=d*vec4(x,0.,1.);}");
    ::STRINGS[39] = uString::Const("uniform float h,i,j,k,l,m;uniform vec2 p,q,r;uniform sampler2D u;varying float s;varying vec2 t;void main(){vec4 w=texture2D(u,vec2(s,.5));vec4 x=vec4(w.xyz,w.w*(clamp(.5-(((abs((length(t)-h)-i)-j)*k)*l),float(0),float(1))*clamp(.5-((min(dot(t,p),min(dot(t,q),dot(t,r)))*k)*l),float(0),float(1))));gl_FragColor=vec4((x.xyz*x.w)*m,x.w*m);}");
    ::STRINGS[4] = uString::Const("a");
    ::STRINGS[2] = uString::Const("b");
    ::STRINGS[3] = uString::Const("c");
    ::STRINGS[5] = uString::Const("d");
    ...
    ::STRINGS[42] = uString::Const("uniform vec2 d,e,g,h,i,j,k,m,l;uniform mat4 f;attribute vec2 a;varying vec2 z,A,B;void main(){vec2 D=a*d;vec2 E=D+e;vec2 F=((((g*(E/g))-h)/i)*j)+k;z=m+(F*l);A=F;B=D;gl_Position=f*vec4(E,0.,1.);}");
    ::STRINGS[43] = uString::Const("uniform vec2 o,n,w,x,y;uniform vec4 p;uniform float q,r,s,t,u,v;uniform sampler2D C;varying vec2 z,A,B;void main(){vec4 D=(c?vec4(float(0)):texture2D(C,b?(o+(fract(z)*n)):A))*p;vec4 E=vec4(D.xyz,D.w*(clamp(.5-(((abs((length(B)-q)-r)-s)*t)*u),float(0),float(1))*clamp(.5-((min(dot(B,w),min(dot(B,x),dot(B,y)))*t)*u),float(0),float(1))));gl_FragColor=vec4((E.xyz*E.w)*v,E.w*v);}");
    ...
    ::STRINGS[50] = uString::Const("uniform vec2 b,c,e,f,g,l;uniform mat4 d;uniform float m;attribute vec2 a;varying float q;varying vec2 r;float t(vec2 u,vec2 v,float w){vec2 x=(u*e)-f;float y=dot(x,v)/w;return (y-g.x)/g.y;}void main(){vec2 u=a*b;vec2 v=u+c;q=t(v/e,l,m);r=u;gl_Position=d*vec4(v,0.,1.);}");
    ::STRINGS[51] = uString::Const("uniform float h,i,j,k;uniform vec2 n,o,p;uniform sampler2D s;varying float q;varying vec2 r;void main(){vec4 u=texture2D(s,vec2(q,.5));vec4 v=vec4(u.xyz,u.w*(clamp(.5-(((length(r)-h)*i)*j),float(0),float(1))*clamp(.5-((max(dot(r,n),max(dot(r,o),dot(r,p)))*i)*j),float(0),float(1))));gl_FragColor=vec4((v.xyz*v.w)*k,v.w*k);}");
    ::STRINGS[52] = uString::Const("uniform vec2 b,c,e,f,g,l;uniform mat4 d;uniform float m;attribute vec2 a;varying float n;varying vec2 o;float q(vec2 r,vec2 s,float t){vec2 u=(r*e)-f;float v=dot(u,s)/t;return (v-g.x)/g.y;}void main(){vec2 r=a*b;vec2 s=r+c;n=q(s/e,l,m);o=r;gl_Position=d*vec4(s,0.,1.);}");
    ::STRINGS[53] = uString::Const("uniform float h,i,j,k;uniform sampler2D p;varying float n;varying vec2 o;void main(){vec4 r=texture2D(p,vec2(n,.5));vec4 s=vec4(r.xyz,r.w*(clamp(.5-(((length(o)-h)*i)*j),float(0),float(1))*float(1)));gl_FragColor=vec4((s.xyz*s.w)*k,s.w*k);}");
    ::STRINGS[54] = uString::Const("uniform float h,i,j,k;uniform vec2 n,o,p;uniform sampler2D s;varying float q;varying vec2 r;void main(){vec4 u=texture2D(s,vec2(q,.5));vec4 v=vec4(u.xyz,u.w*(clamp(.5-(((length(r)-h)*i)*j),float(0),float(1))*clamp(.5-((min(dot(r,n),min(dot(r,o),dot(r,p)))*i)*j),float(0),float(1))));gl_FragColor=vec4((v.xyz*v.w)*k,v.w*k);}");
    ::STRINGS[55] = uString::Const("uniform float h,i,j,k,l,m;uniform vec2 p,q,r;uniform sampler2D u;varying float s;varying vec2 t;void main(){vec4 w=texture2D(u,vec2(s,.5));vec4 x=vec4(w.xyz,w.w*(clamp(.5-(((abs((length(t)-h)-i)-j)*k)*l),float(0),float(1))*clamp(.5-((max(dot(t,p),max(dot(t,q),dot(t,r)))*k)*l),float(0),float(1))));gl_FragColor=vec4((x.xyz*x.w)*m,x.w*m);}");
    ::STRINGS[56] = uString::Const("uniform vec2 b,c,e,f,g,n;uniform mat4 d;uniform float o;attribute vec2 a;varying float p;varying vec2 q;float s(vec2 t,vec2 u,float v){vec2 w=(t*e)-f;float x=dot(w,u)/v;return (x-g.x)/g.y;}void main(){vec2 t=a*b;vec2 u=t+c;p=s(u/e,n,o);q=t;gl_Position=d*vec4(u,0.,1.);}");
    ::STRINGS[57] = uString::Const("uniform float h,i,j,k,l,m;uniform sampler2D r;varying float p;varying vec2 q;void main(){vec4 t=texture2D(r,vec2(p,.5));vec4 u=vec4(t.xyz,t.w*(clamp(.5-(((abs((length(q)-h)-i)-j)*k)*l),float(0),float(1))*float(1)));gl_FragColor=vec4((u.xyz*u.w)*m,u.w*m);}");
    ::STRINGS[58] = uString::Const("uniform vec2 b,c;uniform mat4 d;attribute vec2 a;varying vec2 l;void main(){vec2 m=a*b;l=m;gl_Position=d*vec4(m+c,0.,1.);}");
    ::STRINGS[59] = uString::Const("uniform float e,f,g,h,i,j;uniform vec3 k;varying vec2 l;void main(){vec4 m=vec4(k,j*(clamp(.5-(((abs((length(l)-e)-f)-g)*h)*i),float(0),float(1))*float(1)));gl_FragColor=vec4((m.xyz*m.w)*1.,m.w);}");
    ::STRINGS[60] = uString::Const("uniform vec2 b,c;uniform mat4 d;attribute vec2 a;varying vec2 p;void main(){vec2 q=a*b;p=q;gl_Position=d*vec4(q+c,0.,1.);}");
    ::STRINGS[61] = uString::Const("uniform float e,f,g,h,i,j,l;uniform vec2 m,n,o;uniform vec3 k;varying vec2 p;void main(){vec4 q=vec4(k,j*(clamp(.5-(((abs((length(p)-e)-f)-g)*h)*i),float(0),float(1))*clamp(.5-((min(dot(p,m),min(dot(p,n),dot(p,o)))*h)*i),float(0),float(1))));gl_FragColor=vec4((q.xyz*q.w)*l,q.w*l);}");
    ::STRINGS[62] = uString::Const("uniform vec2 o,n,w,x,y;uniform vec4 p;uniform float q,r,s,t,u,v;uniform sampler2D C;varying vec2 z,A,B;void main(){vec4 D=(c?vec4(float(0)):texture2D(C,b?(o+(fract(z)*n)):A))*p;vec4 E=vec4(D.xyz,D.w*(clamp(.5-(((abs((length(B)-q)-r)-s)*t)*u),float(0),float(1))*clamp(.5-((max(dot(B,w),max(dot(B,x),dot(B,y)))*t)*u),float(0),float(1))));gl_FragColor=vec4((E.xyz*E.w)*v,E.w*v);}");
    ::STRINGS[63] = uString::Const("uniform vec2 b,c;uniform mat4 d;attribute vec2 a;varying vec2 o;void main(){vec2 p=a*b;o=p;gl_Position=d*vec4(p+c,0.,1.);}");
    ::STRINGS[64] = uString::Const("uniform float e,f,g,h,i,j;uniform vec2 l,m,n;uniform vec3 k;varying vec2 o;void main(){vec4 p=vec4(k,j*(clamp(.5-(((abs((length(o)-e)-f)-g)*h)*i),float(0),float(1))*clamp(.5-((min(dot(o,l),min(dot(o,m),dot(o,n)))*h)*i),float(0),float(1))));gl_FragColor=vec4((p.xyz*p.w)*1.,p.w);}");
    ::STRINGS[65] = uString::Const("uniform vec2 b,c;uniform mat4 d;attribute vec2 a;varying vec2 m;void main(){vec2 n=a*b;m=n;gl_Position=d*vec4(n+c,0.,1.);}");
    ::STRINGS[66] = uString::Const("uniform float e,f,g,h,i,j,l;uniform vec3 k;varying vec2 m;void main(){vec4 n=vec4(k,j*(clamp(.5-(((abs((length(m)-e)-f)-g)*h)*i),float(0),float(1))*float(1)));gl_FragColor=vec4((n.xyz*n.w)*l,n.w*l);}");
    ::STRINGS[67] = uString::Const("uniform vec2 d,e,g,h,i,j,k,m,l;uniform mat4 f;attribute vec2 a;varying vec2 w,x,y;void main(){vec2 A=a*d;vec2 B=A+e;vec2 C=((((g*(B/g))-h)/i)*j)+k;w=m+(C*l);x=C;y=A;gl_Position=f*vec4(B,0.,1.);}");
    ::STRINGS[68] = uString::Const("uniform vec2 o,n;uniform vec4 p;uniform float q,r,s,t,u,v;uniform sampler2D z;varying vec2 w,x,y;void main(){vec4 A=(c?vec4(float(0)):texture2D(z,b?(o+(fract(w)*n)):x))*p;vec4 B=vec4(A.xyz,A.w*(clamp(.5-(((abs((length(y)-q)-r)-s)*t)*u),float(0),float(1))*float(1)));gl_FragColor=vec4((B.xyz*B.w)*v,B.w*v);}");
    ::STRINGS[69] = uString::Const("z");
    ::STRINGS[70] = uString::Const("uniform vec2 d,e,g,h,i,j,k,m,l;uniform mat4 f;attribute vec2 a;varying vec2 x,y,z;void main(){vec2 B=a*d;vec2 C=B+e;vec2 D=((((g*(C/g))-h)/i)*j)+k;x=m+(D*l);y=D;z=B;gl_Position=f*vec4(C,0.,1.);}");
    ::STRINGS[71] = uString::Const("uniform vec2 o,n,u,v,w;uniform vec4 p;uniform float q,r,s,t;uniform sampler2D A;varying vec2 x,y,z;void main(){vec4 B=(c?vec4(float(0)):texture2D(A,b?(o+(fract(x)*n)):y))*p;vec4 C=vec4(B.xyz,B.w*(clamp(.5-(((length(z)-q)*r)*s),float(0),float(1))*clamp(.5-((min(dot(z,u),min(dot(z,v),dot(z,w)))*r)*s),float(0),float(1))));gl_FragColor=vec4((C.xyz*C.w)*t,C.w*t);}");
    ::STRINGS[72] = uString::Const("A");
    ::STRINGS[73] = uString::Const("uniform vec2 d,e,g,h,i,j,k,m,l;uniform mat4 f;attribute vec2 a;varying vec2 u,v,w;void main(){vec2 y=a*d;vec2 z=y+e;vec2 A=((((g*(z/g))-h)/i)*j)+k;u=m+(A*l);v=A;w=y;gl_Position=f*vec4(z,0.,1.);}");
    ::STRINGS[74] = uString::Const("uniform vec2 o,n;uniform vec4 p;uniform float q,r,s,t;uniform sampler2D x;varying vec2 u,v,w;void main(){vec4 y=(c?vec4(float(0)):texture2D(x,b?(o+(fract(u)*n)):v))*p;vec4 z=vec4(y.xyz,y.w*(clamp(.5-(((length(w)-q)*r)*s),float(0),float(1))*float(1)));gl_FragColor=vec4((z.xyz*z.w)*t,z.w*t);}");
    ::STRINGS[75] = uString::Const("uniform vec2 o,n,u,v,w;uniform vec4 p;uniform float q,r,s,t;uniform sampler2D A;varying vec2 x,y,z;void main(){vec4 B=(c?vec4(float(0)):texture2D(A,b?(o+(fract(x)*n)):y))*p;vec4 C=vec4(B.xyz,B.w*(clamp(.5-(((length(z)-q)*r)*s),float(0),float(1))*clamp(.5-((max(dot(z,u),max(dot(z,v),dot(z,w)))*r)*s),float(0),float(1))));gl_FragColor=vec4((C.xyz*C.w)*t,C.w*t);}");
    ::STRINGS[76] = uString::Const("uniform vec2 b,c;uniform mat4 d;attribute vec2 a;varying vec2 k;void main(){vec2 l=a*b;k=l;gl_Position=d*vec4(l+c,0.,1.);}");
    ::STRINGS[77] = uString::Const("uniform float e,f,g,h,j;uniform vec3 i;varying vec2 k;void main(){vec4 l=vec4(i,h*(clamp(.5-(((length(k)-e)*f)*g),float(0),float(1))*float(1)));gl_FragColor=vec4((l.xyz*l.w)*j,l.w*j);}");
    ::STRINGS[78] = uString::Const("uniform float e,f,g,h,i,j;uniform vec2 l,m,n;uniform vec3 k;varying vec2 o;void main(){vec4 p=vec4(k,j*(clamp(.5-(((abs((length(o)-e)-f)-g)*h)*i),float(0),float(1))*clamp(.5-((max(dot(o,l),max(dot(o,m),dot(o,n)))*h)*i),float(0),float(1))));gl_FragColor=vec4((p.xyz*p.w)*1.,p.w);}");
    ::STRINGS[79] = uString::Const("uniform vec2 b,c;uniform mat4 d;attribute vec2 a;varying vec2 n;void main(){vec2 o=a*b;n=o;gl_Position=d*vec4(o+c,0.,1.);}");
    ::STRINGS[80] = uString::Const("uniform float e,f,g,h,j;uniform vec2 k,l,m;uniform vec3 i;varying vec2 n;void main(){vec4 o=vec4(i,h*(clamp(.5-(((length(n)-e)*f)*g),float(0),float(1))*clamp(.5-((max(dot(n,k),max(dot(n,l),dot(n,m)))*f)*g),float(0),float(1))));gl_FragColor=vec4((o.xyz*o.w)*j,o.w*j);}");
    ::STRINGS[81] = uString::Const("uniform float e,f,g,h,j;uniform vec2 k,l,m;uniform vec3 i;varying vec2 n;void main(){vec4 o=vec4(i,h*(clamp(.5-(((length(n)-e)*f)*g),float(0),float(1))*clamp(.5-((min(dot(n,k),min(dot(n,l),dot(n,m)))*f)*g),float(0),float(1))));gl_FragColor=vec4((o.xyz*o.w)*j,o.w*j);}");
    ::STRINGS[82] = uString::Const("uniform float e,f,g,h,i,j,l;uniform vec2 m,n,o;uniform vec3 k;varying vec2 p;void main(){vec4 q=vec4(k,j*(clamp(.5-(((abs((length(p)-e)-f)-g)*h)*i),float(0),float(1))*clamp(.5-((max(dot(p,m),max(dot(p,n),dot(p,o)))*h)*i),float(0),float(1))));gl_FragColor=vec4((q.xyz*q.w)*l,q.w*l);}");
    ::STRINGS[83] = uString::Const("uniform vec2 b,c;uniform mat4 d;attribute vec2 a;varying vec2 j;void main(){vec2 k=a*b;j=k;gl_Position=d*vec4(k+c,0.,1.);}");
    ::STRINGS[84] = uString::Const("uniform float e,f,g,h;uniform vec3 i;varying vec2 j;void main(){vec4 k=vec4(i,h*(clamp(.5-(((length(j)-e)*f)*g),float(0),float(1))*float(1)));gl_FragColor=vec4((k.xyz*k.w)*1.,k.w);}");
    ::STRINGS[85] = uString::Const("uniform float e,f,g,h;uniform vec2 j,k,l;uniform vec3 i;varying vec2 m;void main(){vec4 n=vec4(i,h*(clamp(.5-(((length(m)-e)*f)*g),float(0),float(1))*clamp(.5-((min(dot(m,j),min(dot(m,k),dot(m,l)))*f)*g),float(0),float(1))));gl_FragColor=vec4((n.xyz*n.w)*1.,n.w);}");
    ::STRINGS[86] = uString::Const("uniform float e,f,g,h;uniform vec2 j,k,l;uniform vec3 i;varying vec2 m;void main(){vec4 n=vec4(i,h*(clamp(.5-(((length(m)-e)*f)*g),float(0),float(1))*clamp(.5-((max(dot(m,j),max(dot(m,k),dot(m,l)))*f)*g),float(0),float(1))));gl_FragColor=vec4((n.xyz*n.w)*1.,n.w);}");
    ::STRINGS[87] = uString::Const("uniform float e[d],o;uniform vec2 f,h,i,j,n;uniform mat4 g;attribute vec4 a,b;attribute float c;varying float p,q;varying vec2 r,s;float u(vec2 v,vec2 w,float x){vec2 y=(v*h)-i;float z=dot(y,w)/x;return (z-j.x)/j.y;}void main(){vec2 v=vec2((sign(a.x)*e[int(abs(a.x))])+(sign(a.y)*e[int(abs(a.y))]),(sign(a.z)*e[int(abs(a.z))])+(sign(a.w)*e[int(abs(a.w))]))+f;p=e[int(c)];q=u(v/h,n,o);r=v;s=vec2((sign(b.x)*e[int(abs(b.x))])+(sign(b.y)*e[int(abs(b.y))]),(sign(b.z)*e[int(abs(b.z))])+(sign(b.w)*e[int(abs(b.w))]))+f;gl_Position=g*vec4(v,0.,1.);}");
    ::STRINGS[88] = uString::Const("uniform float k,l,m;uniform sampler2D t;varying float p,q;varying vec2 r,s;void main(){vec4 v=texture2D(t,vec2(q,.5));vec4 w=vec4(v.xyz,v.w*clamp(.5-(((distance(r,s)-p)*k)*l),float(0),float(1)));gl_FragColor=vec4((w.xyz*w.w)*m,w.w*m);}");
    ::STRINGS[89] = uString::Const("uniform float k,l,m;uniform sampler2D t;varying float p,q;varying vec2 r,s;void main(){vec4 v=texture2D(t,vec2(q,.5));float w=clamp(.5-(((distance(r,s)-p)*k)*l),float(0),float(1));vec4 x=vec4(v.xyz,v.w*(((w*w)*w)*((w*((w*float(6))-float(15)))+float(10))));gl_FragColor=vec4((x.xyz*x.w)*m,x.w*m);}");
    ::STRINGS[90] = uString::Const("uniform float e[d];uniform vec2 f;uniform mat4 g;attribute vec4 a,b;attribute float c;varying float l;varying vec2 m,n;void main(){vec2 o=vec2((sign(a.x)*e[int(abs(a.x))])+(sign(a.y)*e[int(abs(a.y))]),(sign(a.z)*e[int(abs(a.z))])+(sign(a.w)*e[int(abs(a.w))]))+f;l=e[int(c)];m=o;n=vec2((sign(b.x)*e[int(abs(b.x))])+(sign(b.y)*e[int(abs(b.y))]),(sign(b.z)*e[int(abs(b.z))])+(sign(b.w)*e[int(abs(b.w))]))+f;gl_Position=g*vec4(o,0.,1.);}");
    ::STRINGS[91] = uString::Const("uniform float h,i,j;uniform vec3 k;varying float l;varying vec2 m,n;void main(){vec4 o=vec4(k,j*clamp(.5-(((distance(m,n)-l)*h)*i),float(0),float(1)));gl_FragColor=vec4((o.xyz*o.w)*1.,o.w);}");
    ::STRINGS[92] = uString::Const("uniform float h,i,j;uniform vec3 k;varying float l;varying vec2 m,n;void main(){float o=clamp(.5-(((distance(m,n)-l)*h)*i),float(0),float(1));vec4 p=vec4(k,j*(((o*o)*o)*((o*((o*float(6))-float(15)))+float(10))));gl_FragColor=vec4((p.xyz*p.w)*1.,p.w);}");
    ::STRINGS[93] = uString::Const("uniform float g[d];uniform vec2 h,j,k,l,m,n,p,o;uniform mat4 i;attribute vec4 a,b;attribute float c;varying float w;varying vec2 x,y,z,A;void main(){vec2 C=vec2((sign(a.x)*g[int(abs(a.x))])+(sign(a.y)*g[int(abs(a.y))]),(sign(a.z)*g[int(abs(a.z))])+(sign(a.w)*g[int(abs(a.w))]))+h;vec2 D=((((j*(C/j))-k)/l)*m)+n;w=g[int(c)];x=p+(D*o);y=D;z=C;A=vec2((sign(b.x)*g[int(abs(b.x))])+(sign(b.y)*g[int(abs(b.y))]),(sign(b.z)*g[int(abs(b.z))])+(sign(b.w)*g[int(abs(b.w))]))+h;gl_Position=i*vec4(C,0.,1.);}");
    ::STRINGS[94] = uString::Const("uniform vec2 r,q;uniform vec4 s;uniform float t,u,v;uniform sampler2D B;varying float w;varying vec2 x,y,z,A;void main(){vec4 C=(f?vec4(float(0)):texture2D(B,e?(r+(fract(x)*q)):y))*s;vec4 D=vec4(C.xyz,C.w*clamp(.5-(((distance(z,A)-w)*t)*u),float(0),float(1)));gl_FragColor=vec4((D.xyz*D.w)*v,D.w*v);}");
    ::STRINGS[95] = uString::Const("B");
    ::STRINGS[96] = uString::Const("uniform vec2 r,q;uniform vec4 s;uniform float t,u,v;uniform sampler2D B;varying float w;varying vec2 x,y,z,A;void main(){vec4 C=(f?vec4(float(0)):texture2D(B,e?(r+(fract(x)*q)):y))*s;float D=clamp(.5-(((distance(z,A)-w)*t)*u),float(0),float(1));vec4 E=vec4(C.xyz,C.w*(((D*D)*D)*((D*((D*float(6))-float(15)))+float(10))));gl_FragColor=vec4((E.xyz*E.w)*v,E.w*v);}");
    ::STRINGS[97] = uString::Const("uniform float e[d];uniform vec2 f;uniform mat4 g;attribute vec4 a,b;attribute float c;varying float n;varying vec2 o,p;void main(){vec2 q=vec2((sign(a.x)*e[int(abs(a.x))])+(sign(a.y)*e[int(abs(a.y))]),(sign(a.z)*e[int(abs(a.z))])+(sign(a.w)*e[int(abs(a.w))]))+f;n=e[int(c)];o=q;p=vec2((sign(b.x)*e[int(abs(b.x))])+(sign(b.y)*e[int(abs(b.y))]),(sign(b.z)*e[int(abs(b.z))])+(sign(b.w)*e[int(abs(b.w))]))+f;gl_Position=g*vec4(q,0.,1.);}");
    ::STRINGS[98] = uString::Const("uniform float h,i,j,k,l;uniform vec3 m;varying float n;varying vec2 o,p;void main(){vec4 q=vec4(m,l*clamp(.5-(((abs((distance(o,p)-n)-h)-i)*j)*k),float(0),float(1)));gl_FragColor=vec4((q.xyz*q.w)*1.,q.w);}");
    ...
    ::STRINGS[100] = uString::Const("uniform float h,i,j,l;uniform vec3 k;varying float m;varying vec2 n,o;void main(){vec4 p=vec4(k,j*clamp(.5-(((distance(n,o)-m)*h)*i),float(0),float(1)));gl_FragColor=vec4((p.xyz*p.w)*l,p.w*l);}");
    ::STRINGS[101] = uString::Const("uniform float h,i,j,l;uniform vec3 k;varying float m;varying vec2 n,o;void main(){float p=clamp(.5-(((distance(n,o)-m)*h)*i),float(0),float(1));vec4 q=vec4(k,j*(((p*p)*p)*((p*((p*float(6))-float(15)))+float(10))));gl_FragColor=vec4((q.xyz*q.w)*l,q.w*l);}");
    ::STRINGS[102] = uString::Const("uniform float h,i,j,k,l;uniform vec3 m;varying float n;varying vec2 o,p;void main(){float q=clamp(.5-(((abs((distance(o,p)-n)-h)-i)*j)*k),float(0),float(1));vec4 r=vec4(m,l*(((q*q)*q)*((q*((q*float(6))-float(15)))+float(10))));gl_FragColor=vec4((r.xyz*r.w)*1.,r.w);}");
    ::STRINGS[103] = uString::Const("uniform float g[d];uniform vec2 h,j,k,l,m,n,p,o;uniform mat4 i;attribute vec4 a,b;attribute float c;varying float y;varying vec2 z,A,B,C;void main(){vec2 E=vec2((sign(a.x)*g[int(abs(a.x))])+(sign(a.y)*g[int(abs(a.y))]),(sign(a.z)*g[int(abs(a.z))])+(sign(a.w)*g[int(abs(a.w))]))+h;vec2 F=((((j*(E/j))-k)/l)*m)+n;y=g[int(c)];z=p+(F*o);A=F;B=E;C=vec2((sign(b.x)*g[int(abs(b.x))])+(sign(b.y)*g[int(abs(b.y))]),(sign(b.z)*g[int(abs(b.z))])+(sign(b.w)*g[int(abs(b.w))]))+h;gl_Position=i*vec4(E,0.,1.);}");
    ::STRINGS[104] = uString::Const("uniform vec2 r,q;uniform vec4 s;uniform float t,u,v,w,x;uniform sampler2D D;varying float y;varying vec2 z,A,B,C;void main(){vec4 E=(f?vec4(float(0)):texture2D(D,e?(r+(fract(z)*q)):A))*s;float F=clamp(.5-(((abs((distance(B,C)-y)-t)-u)*v)*w),float(0),float(1));vec4 G=vec4(E.xyz,E.w*(((F*F)*F)*((F*((F*float(6))-float(15)))+float(10))));gl_FragColor=vec4((G.xyz*G.w)*x,G.w*x);}");
    ::STRINGS[105] = uString::Const("D");
    ::STRINGS[106] = uString::Const("uniform vec2 r,q;uniform vec4 s;uniform float t,u,v,w,x;uniform sampler2D D;varying float y;varying vec2 z,A,B,C;void main(){vec4 E=(f?vec4(float(0)):texture2D(D,e?(r+(fract(z)*q)):A))*s;vec4 F=vec4(E.xyz,E.w*clamp(.5-(((abs((distance(B,C)-y)-t)-u)*v)*w),float(0),float(1)));gl_FragColor=vec4((F.xyz*F.w)*x,F.w*x);}");
    ::STRINGS[107] = uString::Const("uniform float e[d];uniform vec2 f;uniform mat4 g;attribute vec4 a,b;attribute float c;varying float o;varying vec2 p,q;void main(){vec2 r=vec2((sign(a.x)*e[int(abs(a.x))])+(sign(a.y)*e[int(abs(a.y))]),(sign(a.z)*e[int(abs(a.z))])+(sign(a.w)*e[int(abs(a.w))]))+f;o=e[int(c)];p=r;q=vec2((sign(b.x)*e[int(abs(b.x))])+(sign(b.y)*e[int(abs(b.y))]),(sign(b.z)*e[int(abs(b.z))])+(sign(b.w)*e[int(abs(b.w))]))+f;gl_Position=g*vec4(r,0.,1.);}");
    ::STRINGS[108] = uString::Const("uniform float h,i,j,k,l,n;uniform vec3 m;varying float o;varying vec2 p,q;void main(){float r=clamp(.5-(((abs((distance(p,q)-o)-h)-i)*j)*k),float(0),float(1));vec4 s=vec4(m,l*(((r*r)*r)*((r*((r*float(6))-float(15)))+float(10))));gl_FragColor=vec4((s.xyz*s.w)*n,s.w*n);}");
    ::STRINGS[109] = uString::Const("uniform float h,i,j,k,l,n;uniform vec3 m;varying float o;varying vec2 p,q;void main(){vec4 r=vec4(m,l*clamp(.5-(((abs((distance(p,q)-o)-h)-i)*j)*k),float(0),float(1)));gl_FragColor=vec4((r.xyz*r.w)*n,r.w*n);}");
    ::STRINGS[110] = uString::Const("uniform float e[d],q;uniform vec2 f,h,i,j,p;uniform mat4 g;attribute vec4 a,b;attribute float c;varying float r,s;varying vec2 t,u;float w(vec2 x,vec2 y,float z){vec2 A=(x*h)-i;float B=dot(A,y)/z;return (B-j.x)/j.y;}void main(){vec2 x=vec2((sign(a.x)*e[int(abs(a.x))])+(sign(a.y)*e[int(abs(a.y))]),(sign(a.z)*e[int(abs(a.z))])+(sign(a.w)*e[int(abs(a.w))]))+f;r=e[int(c)];s=w(x/h,p,q);t=x;u=vec2((sign(b.x)*e[int(abs(b.x))])+(sign(b.y)*e[int(abs(b.y))]),(sign(b.z)*e[int(abs(b.z))])+(sign(b.w)*e[int(abs(b.w))]))+f;gl_Position=g*vec4(x,0.,1.);}");
    ::STRINGS[111] = uString::Const("uniform float k,l,m,n,o;uniform sampler2D v;varying float r,s;varying vec2 t,u;void main(){vec4 x=texture2D(v,vec2(s,.5));vec4 y=vec4(x.xyz,x.w*clamp(.5-(((abs((distance(t,u)-r)-k)-l)*m)*n),float(0),float(1)));gl_FragColor=vec4((y.xyz*y.w)*o,y.w*o);}");
    ::STRINGS[112] = uString::Const("uniform float k,l,m,n,o;uniform sampler2D v;varying float r,s;varying vec2 t,u;void main(){vec4 x=texture2D(v,vec2(s,.5));float y=clamp(.5-(((abs((distance(t,u)-r)-k)-l)*m)*n),float(0),float(1));vec4 z=vec4(x.xyz,x.w*(((y*y)*y)*((y*((y*float(6))-float(15)))+float(10))));gl_FragColor=vec4((z.xyz*z.w)*o,z.w*o);}");
    ::TYPES[0] = ::g::Uno::String_typeof()->Array();
    type->SetFields(0,
...
// C:\Users\EliteBook-User\AppData\Local\Fusetools\Packages\Fuse.Effects\0.44.1\.uno\package
// -----------------------------------------------------------------------------------------
...
static void FuseEffects_bundle__cctor__fn(uType* __type)
{
    ...
    FuseEffects_bundle::Desaturate88bb7f94_ = ::g::Uno::Runtime::Implementation::ShaderBackends::OpenGL::GLProgram::Create(::STRINGS[26/*"uniform mat...*/], ::STRINGS[115/*"uniform flo...*/], 0, 1, uArray::Init<uString*>(::TYPES[0/*string[]*/], 5, ::STRINGS[4/*"a"*/], ::STRINGS[2/*"b"*/], ::STRINGS[3/*"c"*/], ::STRINGS[5/*"d"*/], ::STRINGS[7/*"f"*/]));
    FuseEffects_bundle::EffectHelpers52052d40_ = ::g::Uno::Runtime::Implementation::ShaderBackends::OpenGL::GLProgram::Create(::STRINGS[116/*"attribute v...*/], ::STRINGS[117/*"uniform flo...*/], 3, 1, uArray::Init<uString*>(::TYPES[0/*string[]*/], 7, ::STRINGS[2/*"b"*/], ::STRINGS[3/*"c"*/], ::STRINGS[5/*"d"*/], ::STRINGS[4/*"a"*/], ::STRINGS[6/*"e"*/], ::STRINGS[7/*"f"*/], ::STRINGS[9/*"h"*/]));
    FuseEffects_bundle::EffectHelpers9b892494_ = ::g::Uno::Runtime::Implementation::ShaderBackends::OpenGL::GLProgram::Create(::STRINGS[118/*"attribute v...*/], ::STRINGS[119/*"uniform sam...*/], 0, 1, uArray::Init<uString*>(::TYPES[0/*string[]*/], 2, ::STRINGS[4/*"a"*/], ::STRINGS[3/*"c"*/]));
    FuseEffects_bundle::EffectHelpersd8c2e3e5_ = ::g::Uno::Runtime::Implementation::ShaderBackends::OpenGL::GLProgram::Create(::STRINGS[120/*"uniform vec...*/], ::STRINGS[121/*"uniform flo...*/], 0, 1, uArray::Init<uString*>(::TYPES[0/*string[]*/], 11, ::STRINGS[4/*"a"*/], ::STRINGS[2/*"b"*/], ::STRINGS[3/*"c"*/], ::STRINGS[5/*"d"*/], ::STRINGS[6/*"e"*/], ::STRINGS[7/*"f"*/], ::STRINGS[8/*"g"*/], ::STRINGS[9/*"h"*/], ::STRINGS[10/*"i"*/], ::STRINGS[11/*"j"*/], ::STRINGS[19/*"t"*/]));
    FuseEffects_bundle::EffectHelperse7e608d1_ = ::g::Uno::Runtime::Implementation::ShaderBackends::OpenGL::GLProgram::Create(::STRINGS[122/*"uniform vec...*/], ::STRINGS[123/*"uniform flo...*/], 0, 1, uArray::Init<uString*>(::TYPES[0/*string[]*/], 8, ::STRINGS[4/*"a"*/], ::STRINGS[2/*"b"*/], ::STRINGS[3/*"c"*/], ::STRINGS[5/*"d"*/], ::STRINGS[6/*"e"*/], ::STRINGS[7/*"f"*/], ::STRINGS[8/*"g"*/], ::STRINGS[14/*"m"*/]));
    FuseEffects_bundle::Halftone3c4c510c_ = ::g::Uno::Runtime::Implementation::ShaderBackends::OpenGL::GLProgram::Create(::STRINGS[124/*"uniform mat...*/], ::STRINGS[125/*"uniform flo...*/], 0, 1, uArray::Init<uString*>(::TYPES[0/*string[]*/], 14, ::STRINGS[4/*"a"*/], ::STRINGS[2/*"b"*/], ::STRINGS[3/*"c"*/], ::STRINGS[5/*"d"*/], ::STRINGS[6/*"e"*/], ::STRINGS[7/*"f"*/], ::STRINGS[8/*"g"*/], ::STRINGS[9/*"h"*/], ::STRINGS[10/*"i"*/], ::STRINGS[11/*"j"*/], ::STRINGS[12/*"k"*/], ::STRINGS[13/*"l"*/], ::STRINGS[14/*"m"*/], ::STRINGS[18/*"q"*/]));
    FuseEffects_bundle::Mask0da73af7_ = ::g::Uno::Runtime::Implementation::ShaderBackends::OpenGL::GLProgram::Create(::STRINGS[126/*"uniform mat...*/], ::STRINGS[127/*"uniform sam...*/], 0, 1, uArray::Init<uString*>(::TYPES[0/*string[]*/], 7, ::STRINGS[4/*"a"*/], ::STRINGS[2/*"b"*/], ::STRINGS[3/*"c"*/], ::STRINGS[5/*"d"*/], ::STRINGS[6/*"e"*/], ::STRINGS[9/*"h"*/], ::STRINGS[10/*"i"*/]));
    FuseEffects_bundle::Mask6affc65c_ = ::g::Uno::Runtime::Implementation::ShaderBackends::OpenGL::GLProgram::Create(::STRINGS[126/*"uniform mat...*/], ::STRINGS[128/*"uniform sam...*/], 0, 1, uArray::Init<uString*>(::TYPES[0/*string[]*/], 7, ::STRINGS[4/*"a"*/], ::STRINGS[2/*"b"*/], ::STRINGS[3/*"c"*/], ::STRINGS[5/*"d"*/], ::STRINGS[6/*"e"*/], ::STRINGS[9/*"h"*/], ::STRINGS[10/*"i"*/]));
    FuseEffects_bundle::Maskb04eaf92_ = ::g::Uno::Runtime::Implementation::ShaderBackends::OpenGL::GLProgram::Create(::STRINGS[126/*"uniform mat...*/], ::STRINGS[129/*"uniform sam...*/], 0, 1, uArray::Init<uString*>(::TYPES[0/*string[]*/], 7, ::STRINGS[4/*"a"*/], ::STRINGS[2/*"b"*/], ::STRINGS[3/*"c"*/], ::STRINGS[5/*"d"*/], ::STRINGS[6/*"e"*/], ::STRINGS[9/*"h"*/], ::STRINGS[10/*"i"*/]));
}
...
static void FuseEffects_bundle_build(uType* type)
{
    ...
    ::STRINGS[25] = uString::Const("uniform sampler2D e;varying vec2 d;void main(){gl_FragColor=texture2D(e,d);}");
    ::STRINGS[26] = uString::Const("uniform mat4 d,b;attribute vec2 a;varying vec2 e;void main(){e=true?vec2(a.x,float(1)-a.y):a;gl_Position=b*(d*vec4(vec3(a,float(0)),1.));}");
    ::STRINGS[115] = uString::Const("uniform float c;uniform sampler2D f;varying vec2 e;void main(){vec4 g=texture2D(f,e);vec4 h=vec4(g.xyz/max(g.w,1e-05),g.w);gl_FragColor=vec4(mix(h.xyz,vec3(sqrt(dot(h.xyz*h.xyz,vec3(.299,.587,.114)))),c),h.w);}");
    ::STRINGS[116] = uString::Const("attribute vec2 a;varying vec2 g;void main(){vec3 j=vec3((a.xy*vec2(float(2),float(2)))+vec3(float(-1),float(-1),float(0)).xy,float(0));g=(j.xy*.5)+.5;gl_Position=vec4(j,float(1));}");
    ::STRINGS[117] = uString::Const("uniform float e[b];uniform vec2 f[d];uniform sampler2D h;varying vec2 g;vec4 i(vec2 j){vec4 k=texture2D(h,j)*e[0];for(int l=0;l<c;++l){k=k+(texture2D(h,j+f[l])*e[1+l]);k=k+(texture2D(h,j-f[l])*e[1+l]);}return k;}void main(){gl_FragColor=i(g);}");
    ::STRINGS[118] = uString::Const("attribute vec2 a;varying vec2 b;void main(){vec3 d=vec3((a.xy*vec2(float(2),float(2)))+vec3(float(-1),float(-1),float(0)).xy,float(0));b=(d.xy*.5)+.5;gl_Position=vec4(d,float(1));}");
    ::STRINGS[119] = uString::Const("uniform sampler2D c;varying vec2 b;void main(){gl_FragColor=texture2D(c,b);}");
    ::STRINGS[120] = uString::Const("uniform vec2 c,e,f,h,i,j;attribute vec2 a;varying vec2 k,l,m,n,o,p,q,r,s;void main(){vec3 u=vec3((a.xy*vec2(float(2),float(2)))+vec3(float(-1),float(-1),float(0)).xy,float(0));vec2 v=(u.xy*.5)+.5;k=v;l=v+c;m=v-c;n=v+e;o=v-e;p=v+f;q=v+h;r=v+i;s=v+j;gl_Position=vec4(u,float(1));}");
    ::STRINGS[121] = uString::Const("uniform float b,d,g;uniform sampler2D t;varying vec2 k,l,m,n,o,p,q,r,s;void main(){gl_FragColor=((((((((texture2D(t,k)*b)+(texture2D(t,l)*d))+(texture2D(t,m)*d))+(texture2D(t,n)*d))+(texture2D(t,o)*d))+(texture2D(t,p)*g))+(texture2D(t,q)*g))+(texture2D(t,r)*g))+(texture2D(t,s)*g);}");
    ::STRINGS[8] = uString::Const("g");
    ::STRINGS[10] = uString::Const("i");
    ::STRINGS[11] = uString::Const("j");
    ::STRINGS[19] = uString::Const("t");
    ...
    ::STRINGS[14] = uString::Const("m");
    ::STRINGS[124] = uString::Const("uniform mat4 m,b;uniform vec2 g,h;uniform mat2 i;attribute vec2 a;varying vec2 n,o,p;void main(){vec2 r=true?vec2(a.x,float(1)-a.y):a;vec2 s=(r*g)+h;n=r;o=i*s;p=s;gl_Position=b*(m*vec4(vec3(a,float(0)),1.));}");
    ::STRINGS[125] = uString::Const("uniform float c,d,e,f,k,l;uniform mat2 j;uniform sampler2D q;varying vec2 n,o,p;void main(){vec4 r=texture2D(q,n);gl_FragColor=mix(mix(vec4(float(1),float(1),float(1),r.w),r,c),mix(vec4(float(0),float(0),float(0),r.w),r,d),clamp(.5-((((((float(1)-sqrt((float(1)-dot(r.xyz*r.xyz,vec3(.299,.587,.114)))/3.14159274))*e)*f)-length(p-(j*((floor(o/e)+.5)*e))))*k)*l),float(0),float(1)));}");
    ::STRINGS[12] = uString::Const("k");
    ::STRINGS[13] = uString::Const("l");
    ::STRINGS[18] = uString::Const("q");
    ::STRINGS[126] = uString::Const("uniform mat4 e,b;uniform vec2 c,d;attribute vec2 a;varying vec2 f,g;void main(){vec2 j=true?vec2(a.x,float(1)-a.y):a;f=j;g=(vec2(j.x,float(1)-j.y)*c)+d;gl_Position=b*(e*vec4(vec3(a,float(0)),1.));}");
    ::STRINGS[127] = uString::Const("uniform sampler2D h,i;varying vec2 f,g;void main(){gl_FragColor=texture2D(h,f)*texture2D(i,g).x;}");
    ::STRINGS[128] = uString::Const("uniform sampler2D h,i;varying vec2 f,g;void main(){vec4 j=texture2D(i,g);gl_FragColor=texture2D(h,f)*vec4(j.xyz*j.w,j.w);}");
    ::STRINGS[129] = uString::Const("uniform sampler2D h,i;varying vec2 f,g;void main(){gl_FragColor=texture2D(h,f)*texture2D(i,g).w;}");
    ::TYPES[0] = ::g::Uno::String_typeof()->Array();
    type->SetFields(0,
...
// C:\Users\EliteBook-User\AppData\Local\Fusetools\Packages\Fuse.Elements\0.44.1\.uno\package
// ------------------------------------------------------------------------------------------
...
{
    FuseElements_bundle::CacheHelper230c69b7_ = ::g::Uno::Runtime::Implementation::ShaderBackends::OpenGL::GLProgram::Create(::STRINGS[130/*"uniform vec...*/], ::STRINGS[131/*"uniform flo...*/], 0, 1, uArray::Init<uString*>(::TYPES[0/*string[]*/], 6, ::STRINGS[4/*"a"*/], ::STRINGS[2/*"b"*/], ::STRINGS[3/*"c"*/], ::STRINGS[5/*"d"*/], ::STRINGS[6/*"e"*/], ::STRINGS[8/*"g"*/]));
    FuseElements_bundle::ElementBatchd43eeb4e_ = ::g::Uno::Runtime::Implementation::ShaderBackends::OpenGL::GLProgram::Create(::STRINGS[132/*"uniform mat...*/], ::STRINGS[133/*"uniform sam...*/], 0, 2, uArray::Init<uString*>(::TYPES[0/*string[]*/], 4, ::STRINGS[4/*"a"*/], ::STRINGS[2/*"b"*/], ::STRINGS[3/*"c"*/], ::STRINGS[7/*"f"*/]));
    ...
    FuseElements_bundle::RepeatBaker447c61dc_ = ::g::Uno::Runtime::Implementation::ShaderBackends::OpenGL::GLProgram::Create(::STRINGS[134/*"uniform vec...*/], ::STRINGS[135/*"uniform sam...*/], 0, 1, uArray::Init<uString*>(::TYPES[0/*string[]*/], 4, ::STRINGS[4/*"a"*/], ::STRINGS[2/*"b"*/], ::STRINGS[3/*"c"*/], ::STRINGS[6/*"e"*/]));
...
{
    ::STRINGS[130] = uString::Const("uniform vec2 b;uniform mat4 c,d;attribute vec2 a;varying vec2 f;void main(){f=vec2(a.x,1.-a.y);gl_Position=d*(c*vec4(a*b,float(0),float(1)));}");
    ::STRINGS[131] = uString::Const("uniform float e;uniform sampler2D g;varying vec2 f;void main(){vec4 h=texture2D(g,f);gl_FragColor=vec4(h.xyz*e,h.w*e);}");
    ::STRINGS[4] = uString::Const("a");
    ::STRINGS[2] = uString::Const("b");
    ::STRINGS[3] = uString::Const("c");
    ::STRINGS[5] = uString::Const("d");
    ::STRINGS[6] = uString::Const("e");
    ::STRINGS[8] = uString::Const("g");
    ::STRINGS[132] = uString::Const("uniform mat4 c;attribute vec3 a;attribute vec2 b;varying float d;varying vec2 e;void main(){d=a.z;e=vec2(b.x,1.-b.y);gl_Position=(a.z>float(0))?(c*vec4(a.xy,float(0),float(1))):vec4(float(0),float(0),float(0),float(-1));}");
    ::STRINGS[133] = uString::Const("uniform sampler2D f;varying float d;varying vec2 e;void main(){gl_FragColor=texture2D(f,e)*d;}");
    ::STRINGS[7] = uString::Const("f");
    ::STRINGS[22] = uString::Const("uniform mat4 d,b;attribute vec2 a;void main(){gl_Position=b*(d*vec4(vec3(a,float(0)),1.));}");
    ::STRINGS[23] = uString::Const("uniform vec4 c;void main(){gl_FragColor=c;}");
    ::STRINGS[134] = uString::Const("uniform vec2 b,c;attribute vec2 a;varying vec2 d;void main(){d=(a*b)/c;gl_Position=vec4((a*float(2))-float(1),float(0),float(1));}");
    ::STRINGS[135] = uString::Const("uniform sampler2D e;varying vec2 d;void main(){gl_FragColor=texture2D(e,fract(d));}");
    ::STRINGS[136] = uString::Const("uniform float c,d,e,f,g,h,k,l,m,n;uniform mat4 i,j;uniform vec2 o;attribute vec3 a,b;varying vec2 q;void main(){q=vec2(((a.x*c)+(a.y*k))+(a.z*l),((b.x*f)+(b.y*m))+(b.z*n))/o;gl_Position=j*(i*vec4(vec2(((a.x*c)+(a.y*d))+(a.z*e),((b.x*f)+(b.y*g))+(b.z*h)),float(0),float(1)));}");
    ::STRINGS[137] = uString::Const("uniform vec4 p;uniform sampler2D r;varying vec2 q;void main(){gl_FragColor=texture2D(r,q)*p;}");
    ::STRINGS[9] = uString::Const("h");
    ::STRINGS[10] = uString::Const("i");
    ::STRINGS[11] = uString::Const("j");
    ::STRINGS[12] = uString::Const("k");
...
// C:\Users\EliteBook-User\AppData\Local\Fusetools\Packages\Fuse.Reactive\0.44.1\.uno\package
// ------------------------------------------------------------------------------------------
...
{
    FuseReactive_bundle::Diagnostics5987d316_ = uPtr(::g::Uno::IO::Bundle::Get(::STRINGS[138/*"Fuse.Reactive"*/]))->GetFile(::STRINGS[139/*"diagnostics...*/]);
    FuseReactive_bundle::es6promisemine43b36df_ = uPtr(::g::Uno::IO::Bundle::Get(::STRINGS[138/*"Fuse.Reactive"*/]))->GetFile(::STRINGS[140/*"es6-promise...*/]);
    FuseReactive_bundle::Observable5d4d0737_ = uPtr(::g::Uno::IO::Bundle::Get(::STRINGS[138/*"Fuse.Reactive"*/]))->GetFile(::STRINGS[141/*"observable-...*/]);
    FuseReactive_bundle::setTimeoutb6c12927_ = uPtr(::g::Uno::IO::Bundle::Get(::STRINGS[138/*"Fuse.Reactive"*/]))->GetFile(::STRINGS[142/*"settimeout-...*/]);
}

static void FuseReactive_bundle_build(uType* type)
{
    ::STRINGS[138] = uString::Const("Fuse.Reactive");
    ::STRINGS[139] = uString::Const("diagnostics-c1dc0a33.js");
    ::STRINGS[140] = uString::Const("es6-promise.min-2da2c24c.js");
    ::STRINGS[141] = uString::Const("observable-48e1b932.js");
    ::STRINGS[142] = uString::Const("settimeout-a74449a4.js");
    type->SetFields(0,
        ::g::Uno::IO::BundleFile_typeof(), (uintptr_t)&::g::FuseReactive_bundle::Diagnostics5987d316_, uFieldFlagsStatic,
        ::g::Uno::IO::BundleFile_typeof(), (uintptr_t)&::g::FuseReactive_bundle::es6promisemine43b36df_, uFieldFlagsStatic,
        ::g::Uno::IO::BundleFile_typeof(), (uintptr_t)&::g::FuseReactive_bundle::Observable5d4d0737_, uFieldFlagsStatic,
        ::g::Uno::IO::BundleFile_typeof(), (uintptr_t)&::g::FuseReactive_bundle::setTimeoutb6c12927_, uFieldFlagsStatic);
}
...
uSStrong< ::g::Uno::IO::BundleFile*> FuseReactive_bundle::Diagnostics5987d316_;
uSStrong< ::g::Uno::IO::BundleFile*> FuseReactive_bundle::es6promisemine43b36df_;
uSStrong< ::g::Uno::IO::BundleFile*> FuseReactive_bundle::Observable5d4d0737_;
uSStrong< ::g::Uno::IO::BundleFile*> FuseReactive_bundle::setTimeoutb6c12927_;
// }
